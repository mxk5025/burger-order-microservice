...
GRILLED_ENDPOINT = "http://3.136.108.141/orders/receive_grilled"
...
def do_polling!
  ...
  send_queue_name = 'condiment.fifo'
  ...
  while true do
    ...
    receive_message_result.messages.each do |message|
      ...
      puts 'Kicks off timer to simulate Frying'
      sleep 2
      puts 'Order completed'

      puts 'Sending Grilled Message to Front End'
      post_to_frontend(message.body)
      puts 'Completed request to Front End'

      puts 'Sending order to Condiments'
      sqs.send_message({
...
def post_to_frontend(message_body)
  ...
  HTTP.post(GRILLED_ENDPOINT, json: {
    id: id
  })
end
