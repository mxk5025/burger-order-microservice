...
WRAPPED_ENDPOINT = "http://3.136.108.141/orders/receive_wrapped"
...
def do_polling!
  ...
  receive_queue_name = 'wrapping.fifo'
  ...
  while true do
    ...
    receive_message_result.messages.each do |message|
      ...
      puts 'Sending Wrapped Message to Front End'
      post_to_frontend(message.body)
      puts 'Completed request to Front End'

      puts 'Sending order to Ready'
      sqs.send_message({
...
def post_to_frontend(message_body)
  ...
  HTTP.post(WRAPPED_ENDPOINT, json: {
    id: id
  })
end
