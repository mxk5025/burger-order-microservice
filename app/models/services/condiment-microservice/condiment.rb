...
CONDIMENTS_APPLIED_ENDPOINT = "http://3.136.108.141/orders/receive_condiments_applied"
...
def do_polling!
  ...
  receive_queue_name = 'condiment.fifo'
  receive_queue_url = sqs.get_queue_url(queue_name: receive_queue_name).queue_url

  # Where to tell the sqs client to send messages to
  send_queue_name = 'wrapping.fifo'
  send_queue_url = sqs.get_queue_url(queue_name: send_queue_name).queue_url
  ...
  while true do
    ...
    receive_message_result.messages.each do |message|
      ...
      puts 'Kicks off timer to simulate Condiment Application'
      sleep 2
      puts 'Order completed'

      puts 'Sending condiments applied message to Front End'
      post_to_frontend(message.body)
      puts 'Completed request to Front End'

      puts 'Sending order to Wrapping'
      sqs.send_message({
        queue_url: send_queue_url,
        message_body: message.body,
        message_deduplication_id: SecureRandom.hex,
        message_group_id: 'prod'
      })
      puts 'completed enequeue'
    end
  end
end
...
def post_to_frontend(message_body)
  ...
  HTTP.post(CONDIMENTS_APPLIED_ENDPOINT, json: {
    id: id
  })
end
